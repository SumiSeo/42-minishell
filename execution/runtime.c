...
/*   Updated: 2024/06/17 18:52:32 by sumseo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	runtime_shell(t_parse *cmds_list, char **env_copy, t_data *data)
{
	...
	while (i < pipe_info->total_cmds)
	{
		...
		if (fork_id == 0)
		{
			if (getfile(cmds_list, pipe_info) == -1)
				exit(EXIT_FAILURE);
		...
		i++;
		if (cmds_list->infile_name && cmds_list->infile >= 0)
			close(cmds_list->infile);
		if (cmds_list->outfile_token && cmds_list->outfile >= 0)
